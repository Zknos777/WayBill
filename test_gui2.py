...
def print_document_windows(filename, printer_name):
    # Используем PowerShell для печати с настройками
    command = f'Start-Process -FilePath "{filename}" -Verb Print -ArgumentList "/p /h /t {printer_name}"'
...
def make_card():
    global selected_name, print_condition
...
# Поля для выбора автомобиля (транспортные средства)
vehicle_entries = []
vehicle_labels = []
...
check = tk.Checkbutton(root, text='Распечатать', variable=check_var, command=update_label)
